    ...
    <main class="px-4 sm:px-6 lg:px-20 pb-20">
        ...
        initFlatpickr() {
            flatpickr($refs.datepicker, {
                ...
                <div class="relative h-[70vh] min-h-[550px] w-full rounded-[2.5rem] overflow-hidden shadow-2xl">
                    ...
                    <div class="absolute inset-0 flex flex-col items-center justify-center text-center px-4 pb-32">
                        <h1
                            class="text-5xl md:text-8xl font-black text-white mb-6 tracking-tighter drop-shadow-[0_4px_4px_rgba(0,0,0,0.5)]">
                            Ready. Set. Go.
                        </h1>
                        <p
                            class="text-xl md:text-2xl text-white/95 font-bold max-w-2xl drop-shadow-[0_2px_2px_rgba(0,0,0,0.5)]">
                    ...
                    <div class="absolute bottom-10 left-1/2 -translate-x-1/2 w-[94%] md:w-full max-w-5xl">
                        <div
                            class="bg-white rounded-2xl shadow-2xl p-2 flex flex-col md:flex-row items-stretch md:items-center gap-1 md:gap-0 border border-slate-200/50 backdrop-blur-sm">
                            <!-- Where (Location Dropdown) -->
                            <div
                                class="flex-[1.5] px-8 md:px-10 py-5 flex flex-col justify-center transition-all hover:bg-slate-50 cursor-pointer rounded-t-2xl md:rounded-l-2xl md:rounded-tr-none md:border-r md:border-slate-100 group">
                                <label
                                    class="block text-[11px] md:text-[12px] font-black text-[#0e1133] uppercase tracking-widest mb-2 group-hover:text-[#1173d4] transition-colors flex items-center gap-2">
                            ...
                            <div
                                ...
                                <div class="flex-1 px-8 md:px-10 py-5 transition-all hover:bg-slate-50 cursor-pointer flex flex-col justify-center relative group"
                                    ...
                                    <label
                                        class="block text-[11px] md:text-[12px] font-black text-slate-400 uppercase tracking-widest mb-2 group-hover:text-slate-600 transition-colors flex items-center gap-2">
                                        <span class="material-symbols-outlined text-base">calendar_today</span>
                                        From
                                    </label>
                                    <span class="text-base md:text-lg font-bold text-slate-800"
                                        x-text="$wire.startDate || 'Add dates'"></span>
                                    <input x-ref="datepicker" class="absolute inset-0 opacity-0 cursor-pointer"
                                        readonly />
                                </div>
                            ...
                            <div class="p-2">
                                <button wire:click="refreshSearch"
                                    ...
                                    <span
                                        class="material-symbols-outlined text-3xl md:text-4xl font-bold group-hover:scale-110 transition-transform">search</span>
                                </button>
                            </div>
                        </div>
                    </div>
            ...
            <section class="mb-14 px-2">
                <div
                    class="flex items-center justify-start md:justify-center gap-2 md:gap-8 overflow-x-auto no-scrollbar pb-2">
                    @php
                        $catIcons = [
                            'SUV' => 'directions_car',
                            'Sedan' => 'minor_crash',
                    ...
                    <button class="flex-shrink-0 flex flex-col items-center gap-1 group"
                        ...
                        <div
                            class="w-14 h-10 flex items-center justify-center rounded-lg transition-all {{ !$category ? 'bg-[#0e1133] shadow-md' : 'bg-slate-50 hover:bg-slate-100' }}">
                            <span
                                class="material-symbols-outlined text-[20px] {{ !$category ? 'text-white' : 'text-slate-500' }}">apps</span>
                        </div>
                        <span
                            class="text-[9px] font-black uppercase tracking-widest leading-none {{ !$category ? 'text-[#0e1133] border-b-2 border-[#0e1133]' : 'text-slate-400 group-hover:text-slate-700' }}">All</span>
                    </button>
                    ...
                    @foreach(($options['categories'] ?? []) as $cat)
                        <button class="flex-shrink-0 flex flex-col items-center gap-1 group"
                            ...
                            <div
                                class="w-14 h-10 flex items-center justify-center rounded-lg transition-all {{ $category == $cat ? 'bg-[#0e1133] shadow-md' : 'bg-slate-50 hover:bg-slate-100' }}">
                                <span
                                    class="material-symbols-outlined text-[20px] {{ $category == $cat ? 'text-white' : 'text-slate-500 group-hover:text-slate-800' }}">
                                    {{ $catIcons[$cat] ?? 'directions_car' }}
                                </span>
                            </div>
                            <span
                                class="text-[9px] font-black uppercase tracking-widest leading-none {{ $category == $cat ? 'text-[#0e1133] border-b-2 border-[#0e1133]' : 'text-slate-400 group-hover:text-slate-700' }}">
                                {{ $cat }}
                            </span>
                        </button>
                    @endforeach
                ...
                <section class="mb-20">
                    <div class="flex justify-between items-end mb-8">
                        ...
                        <div class="flex gap-2">
                            <button
                                class="w-10 h-10 rounded-full border border-slate-200 flex items-center justify-center hover:bg-slate-50 transition-colors shadow-sm">
                                        ...
                                        <a href="{{ route('rent.show', $car) }}" class="group block" wire:navigate>
                                            ...
                                            <div class="flex items-center gap-1.5 mt-1 text-sm text-slate-500">
                                                ...
                                                <div class="flex items-center">
                                                    <span
                                                        class="material-symbols-outlined text-xs text-amber-500 font-bold">star</span>
                                            ...
                                            <div class="mt-2">
                                                <span
                                                    class="text-lg font-black text-[#0e1133]">₦{{ number_format($car->daily_price, 0) }}</span>
                    ...
                    <div wire:loading.remove
                        ...
                        <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-x-6 gap-y-12">
                            @forelse($catalog as $car)
                                <a wire:key="car-{{ $car->id }}" href="{{ route('rent.show', $car) }}" class="group flex flex-col" wire:navigate>
                                    <div class="relative aspect-video rounded-3xl overflow-hidden mb-4 bg-slate-100">
                                        ...
                                        <div class="absolute bottom-4 left-4">
                                            <span
                                                class="px-3 py-1 rounded-full bg-white/90 backdrop-blur-sm text-[10px] font-black uppercase tracking-widest text-slate-900 shadow-sm">
                                                {{ $car->category }}
                                            </span>
                                        </div>
                                    </div>
                                    <div class="flex-1 px-1">
                                        ...
                                        <div class="flex items-center gap-3 text-sm font-medium text-slate-50">
                                            <div class="flex items-center gap-1 text-slate-500">
                                                <span
                                                    class="material-symbols-outlined text-base">airline_seat_recline_normal</span>
                                                <span>{{ $car->seats }}</span>
                                            </div>
                                            <div class="flex items-center gap-1 text-slate-500">
                                        ...
                                        <p
                                            class="text-xs font-bold text-[#0e1133] mt-2 group-hover:underline underline-offset-4 decoration-2">
                                            Rent this car →</p>
                                    </div>
                                </a>
                            @empty
