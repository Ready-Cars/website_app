<div class="flex flex-col md:flex-row min-h-screen">
    ...
    <div class="hidden md:flex w-full md:w-1/2 bg-[#0e1133] flex-col justify-center items-center p-8 lg:p-12 relative">
        <a href="{{ route('home') }}" class="absolute top-8 left-8 inline-flex items-center"
            aria-label="{{ config('app.name') }}" wire:navigate>
        ...
        <div class="max-w-md text-center">
            ...
            <p class="text-white/80 mt-4">Access your account to manage bookings, view rental history, and enjoy
                exclusive member benefits.</p>
        </div>
    </div>

    <!-- Mobile Header -->
    <div class="md:hidden sticky top-0 z-50 bg-[#0e1133] w-full px-6 py-4 flex items-center shadow-lg">
        <a href="{{ route('home') }}" class="inline-flex items-center" aria-label="{{ config('app.name') }}" wire:navigate>
            <img src="{{ asset('img.png') }}" alt="{{ config('app.name') }} logo" class="h-8 w-auto object-contain" />
        </a>
    </div>

    <!-- Form side -->
    <div class="w-full md:w-1/2 flex items-center justify-center p-8 lg:p-16 bg-white min-h-screen">
            ...
            <div>
                ...
                <p class="mt-2 text-sm text-gray-600">
                    ...
                    @if (Route::has('register'))
                        <a class="font-medium text-[#1173d4] hover:text-[#0f63b9]" href="{{ route('register') }}"
            ...
            <form method="POST" wire:submit="login" class="mt-4 space-y-6">
                <div class="rounded-md shadow-sm -space-y-px">
                    <div class="relative">
                        ...
                        <input autocomplete="email" id="email" name="email" type="email" wire:model.defer="email"
                ...
                <div class="flex items-center justify-between">
                    <label class="flex items-center">
                        <input wire:model="remember"
                            class="h-4 w-4 rounded border-gray-300 text-[#1173d4] focus:ring-[#1173d4]"
                            type="checkbox" />
                        <span class="ml-2 block text-sm text-gray-900">Remember me</span>
                    </label>
                    <div class="text-sm">
                        @if (Route::has('password.request'))
                            <a class="font-medium text-[#1173d4] hover:text-[#0f63b9]"
                ...
                <div>
                    <button type="submit"
                        class="group relative flex w-full justify-center rounded-md bg-[#1173d4] py-3 px-4 text-sm font-semibold text-white hover:bg-[#0f63b9] focus:outline-none focus:ring-2 focus:ring-[#1173d4] focus:ring-offset-2">Log
                        in</button>
                </div>
            </form>

            <p class="text-center text-sm text-gray-500">© {{ date('Y') }} {{ config('app.name') }}. All rights
                reserved.</p>
        </div>
    </div>
</div>