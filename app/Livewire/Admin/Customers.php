...
class Customers extends Component
{
    ...
    public function ban(int $id): void
    {
        ...
        if ($user->banned_at) return; // already banned
    ...
    {
        $user = User::findOrFail($id);
        if (!$user->banned_at) return; // already unbanned
        $user->banned_at = null;
        $user->save();
    ...
    public function render()
    {
        ...
        if ($selected) {
            try {
                ...
                if ($colClass) {
                    $col = new $colClass();
                    if (method_exists($col, 'setTitle')) { $col = $col->setTitle('Bookings Over Time'); }
                    if (method_exists($col, 'setAnimated')) { $col = $col->setAnimated(true); }
                    if (method_exists($col, 'setDataLabelsEnabled')) { $col = $col->setDataLabelsEnabled(false); }
                ...
                if ($pieClass) {
                    $pie = new $pieClass();
                    if (method_exists($pie, 'setTitle')) { $pie = $pie->setTitle('Status Breakdown'); }
                    if (method_exists($pie, 'legendBottom')) { $pie = $pie->legendBottom(); }
                    if (method_exists($pie, 'setAnimated')) { $pie = $pie->setAnimated(true); }
