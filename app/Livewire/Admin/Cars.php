...
class Cars extends Component
{
    ...
    #[Url(as: 'category')] public string $category = '';
    #[Url(as: 'trans')] public string $transmission = '';
    #[Url(as: 'fuel')] public string $fuel_type = '';
    #[Url(as: 'seats')] public string $seats = '';
    #[Url(as: 'feat')] public string $featured = '';
    #[Url(as: 'min')] public string $minPrice = '';
    #[Url(as: 'max')] public string $maxPrice = '';
    #[Url(as: 'per')] public int $perPage = 10;
    #[Url(as: 'adv')] public bool $showAdvanced = false;
    ...
    public function mount(CarManagementService $service): void
    {
        ...
        if (!in_array($this->perPage, $this->options['perPages'])) $this->perPage = 10;
    ...
    public function persistOptions(CarManagementService $service): void
    {
        try {
            ...
            foreach ($cats as $v) { \App\Models\CarAttributeOption::create(['type'=>'category','value'=>trim($v)]); }
            foreach ($trs as $v) { \App\Models\CarAttributeOption::create(['type'=>'transmission','value'=>trim($v)]); }
            foreach ($fu as $v) { \App\Models\CarAttributeOption::create(['type'=>'fuel','value'=>trim($v)]); }
            $this->loadManagedOptions($service);
    ...
    {
        if (in_array($name, ['q','category','transmission','fuel_type','seats','featured','minPrice','maxPrice','perPage','showAdvanced'])) {
            $this->resetPage();
        }
    }

    public function toggleAdvanced(): void { $this->showAdvanced = !$this->showAdvanced; }
    ...
    public function closeBookings(): void { $this->bookingsOpen = false; $this->bookingsCarId = null; }
